...
if __name__ == "__main__":
    ...
    for res in  Path(".").glob("results_mc*"):
        pickled_results_path = res.joinpath("training_data")
        for dataset in sorted(pickled_results_path.iterdir()):
            for arch_result in dataset.iterdir():
                ...
                for result in arch_result.glob("*.pickle"):
                    ...
                    is_nan = False
                    try:
                        mcc = [(tp * tn - fp * fn) / np.sqrt((tp + fp) * (tp + fn) * (tn + fp) * (tn + fn)) for tp, tn, fp, fn in zip(experiment_results["test_tp"],
                        ...
                        sensitivity = experiment_results["test_tp"][best_idx] / (experiment_results["test_tp"][best_idx] + experiment_results["test_fn"][best_idx])
                        specificity = experiment_results["test_tn"][best_idx] / (experiment_results["test_tn"][best_idx] + experiment_results["test_fp"][best_idx])
