            ...
            for k in evaluated_ks:
                for grid in evaluated_grids:
                    for datadir in datasets.glob("men*"):
                        ...
                        results_path = Path(".", "results_kan_adam__", f"g{grid}_k{k}_entropy{entropy}_smoothing{smoothing}_lr{lrs}_reg", sex)
                        # get the number of features
                        input_size = X.shape[1]
                        # define KAN architectures
                        steps = list(np.linspace(0, 2, 11))

                        kan_archs = [[20, 34, 26, 2]]
                        kan_archs = [[20, 42, 26, 2]]
                        # for first in steps:
                            ...
                            for idx, (train_index, test_index) in enumerate(skf.split(X, y)):
                                ...
                                results = model.fit(dataset, opt="Adam", lr=lrs, lamb=0.001, lamb_entropy=entropy, steps=200,
                                                    ...
                                                    metrics=(
                                                        ...
                                                        test_fp, test_uar
                                                    ), reg_metric="edge_forward_spline_n", loss_fn=torch.nn.CrossEntropyLoss(label_smoothing=smoothing))
                                ...
                                uar_results.append(np.max(results["test_uar"]))
                                # dump results
                                with open(result_dir.joinpath(f'kan_res_{idx+1}.pickle'), "wb") as output_file:
                                    pickle.dump(results, output_file)
                            print("-----------------------------------------------------------")
                            print(f"mean uar - {np.mean(uar_results)}")